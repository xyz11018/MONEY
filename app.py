...
def resolve_ticker(user_input):
    ...
    # 規則 1: 如果是 4 位以上的純數字 (如 6285, 2330)，自動判斷為台股並補上 .TW
    if user_input.isdigit() and len(user_input) >= 4:
        return f"{user_input}.TW"
    
    # 規則 2: 檢查是否包含中文，若有則呼叫 Yahoo API 搜尋代碼
    ...
    # 規則 3: 美股代碼或大盤指數 (如 QQQ, SCHD, ^TWII)，轉大寫直接回傳
...
def fetch_realtime_data(ticker):
    try:
        ...
        if not data.empty:
            ...
            valid_closes = data['Close'].dropna() # 終極防呆：過濾空值
...
num_assets = st.sidebar.number_input("🔢 設定標的數量", value=len(data.get("locked_portfolio", [])) or 4, min_value=1)

# ==========================================
# 5. 主功能：資產動態監控盤
# ==========================================
if app_mode == "📊 資產動態監控盤":
    st.markdown('<h1>🏦 全球資產動態再平衡儀表板</h1>', unsafe_allow_html=True)
    st.info("💡 輸入提示：台股可直接輸入數字（如 `6285`）、美股輸入代碼（如 `QQQ`, `SCHD`）、大盤輸入 `^TWII`。系統會自動解析！")
    
    # 📌 設定區
    with st.expander("⚙️ 點擊展開：調整初始配置與槓桿鎖定", expanded=(not data["locked_portfolio"])):
        ...
        for i in range(int(num_assets)):
            ...
            raw_tk = r_cols[0].text_input(f"tk_{i}", hist["ticker"], label_visibility="collapsed", placeholder="例如: 6285 或 QQQ").strip()
        ...
        if st.button("📌 鎖定初始庫存並存檔", type="primary"):
            if total_pct != 100:
                ...
                if error_tickers:
                    st.error(f"⚠️ 無法識別或抓取以下代碼：{', '.join(error_tickers)}。鎖定失敗，請確認名稱是否正確。")
                else:
    ...
    if data["locked_portfolio"]:
        ...
        if tw_view:
            ...
            for item in tw_view:
                ...
                c[0].metric(item["ticker"].replace('.TW', ''), f"NTD {item['now_p']:.2f}") # 顯示時隱藏 .TW
                ...
                else: c[5].success(f"✅ 平衡 ({real_pct:.1f}%)")
        ...
        if us_view:
            ...
            for item in us_view:
                ...
                c[0].metric(item["ticker"], f"USD {item['now_p']:.1f}") # 美金小數點第1位
                c[1].write(f"持有: {item.get('init_shares', 0):,} 股" if not item["ticker"].startswith("^") else "大盤追蹤")
                c[2].write(f"目標: {item['target_pct']}%")
                c[3].write(f"槓桿: {item.get('leverage', 1.0)}x")
                c[4].write(f"市值: **NTD {int(item['now_val_ntd']):,}**") # 台幣整數
                if abs(diff) > threshold: c[5].warning(f"⚠️ 偏離 {diff:+.1f}%")
                else: c[5].success(f"✅ 平衡 ({real_pct:.1f}%)")
        ...
        if tw_view or us_view:
            ...
            with footer_cols[0]:
                ...
                pie_df = pd.DataFrame([{"tk": r["ticker"].replace('.TW', ''), "val": r["now_val_ntd"]} for r in (tw_view + us_view)])
                fig_pie = px.pie(pie_df, values='val', names='tk', hole=0.4, color_discrete_sequence=px.colors.qualitative.Pastel)
                st.plotly_chart(fig_pie, use_container_width=True)
                
            with footer_cols[1]:
                st.subheader("📊 權重偏差分析")
                bar_df = pd.DataFrame([{"tk": r["ticker"].replace('.TW', ''), "Real": (r["now_val_ntd"]/total_market_val_ntd*100), "Target": r["target_pct"]} for r in (tw_view + us_view)])
...
# ==========================================
    ...
    raw_ticker_input = st.text_input("輸入欲分析代碼：", "2330")
