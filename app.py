...
/* 專業決策指示徽章 */
.badge-buy { display: inline-block; padding: 6px 14px; border-radius: 6px; background-color: #dff7eb; color: #047857; font-weight: 900; font-size: 0.85rem; border: 1px solid #10b981; text-transform: uppercase; letter-spacing: 0.5px;}
...
# ==========================================
        ...
        if current_view_data:
                    ...
                    if item.get("ticker") == "CASH":
                        ...
                        if shares_diff > 0:
                            if lev >= 2.0 and is_bear_cross:
                                ...
                                rebalance_orders.append(f"<li style='margin-bottom:12px; font-size:1.1rem; border-bottom:1px solid #e2e8f0; padding-bottom:8px;'>🛒 <b>{clean_name}</b> ➡️ 權重不足，但因 <span style='badge-hold'>⚠️ 市場寬度跌破均線</span> <b>建議暫停放大槓桿部位</b>。</li>")
                            else:
            ...
            for item in current_view_data:
                ...
                if item.get("ticker") == "CASH":
                    ...
                    c[0].markdown(f"<div class='ticker-display'>{clean_name}</div><div class='stock-name-display'>{zh_name}</div><div class='price-display'>{'NTD' if is_tw_mode else 'USD'} {n_p:.2f}</div><div style='margin-top:6px; font-size:0.95rem; font-weight:900; color:#0f172a;'>📦 庫存: {fmt_money(shares_qty)} 股</div><div class='date-display'>建倉持持持持持時時時時：: {item.get('holding_days', 0)} 天</div>", unsafe_allow_html=True)
                    c[1].markdown(f"<div class='data-label'>建倉毛本金成本 (Cost):</div><div class='data-value'>NTD {fmt_money(item.get('net_buy_cost', 0))}</div><div class='data-label' style='margin-top:12px;'>預估變現淨市值 (M2M Val):</div><div class='data-value'>NTD {fmt_money(item.get('net_real_val', 0))}</div>", unsafe_allow_html=True)
                ...
                with c[5]:
                    ...
                    if item.get("ticker") == "CASH":
                        ...
                        price_ntd = n_p * mult
                        shares_diff = int(diff_val / price_ntd) if price_ntd > 0 else 0
                        if is_trailing_stop: action_msg = f"<div style='margin-top:16px;'><span class='badge-sell' style='background:#b91c1c; color:white;'>SELL ALL 清倉停利</span> <span style='font-weight:900; font-size:1.15rem; color:#0f172a; margin-left:8px;'>{fmt_money(shares_qty)} 股</span></div>"
                        elif shares_diff > 0: 
                            ...
                            else: action_msg = f"<div style='margin-top:16px;'><span class='badge-buy'>🟢 BUY TO OPEN 建倉</span> <span style='font-weight:900; font-size:1.15rem; color:#0f172a; margin-left:8px;'>{fmt_money(shares_diff)} 股</span></div>"
                        elif shares_diff < 0: action_msg = f"<div style='margin-top:16px;'><span class='badge-sell'>🔴 SELL TO CLOSE 平倉</span> <span style='font-weight:900; font-size:1.15rem; color:#0f172a; margin-left:8px;'>{fmt_money(abs(shares_diff))} 股</span></div>"
                        else: action_msg = f"<div style='margin-top:16px;'><span class='badge-hold'>無指示</span></div>"
    ...
    with tab_inject:
        ...
        if add_cash > 0 and current_view_data:
            ...
            st.markdown("<h4 style='color:#0f172a; font-weight:900; margin-top:0;'>🎯 演算法自動化注水配置比例單：</h4>", unsafe_allow_html=True)
                    ...
                    if shortfall_ntd > 0:
                        if item.get("ticker") == "CASH":
                            ...
                            if lev >= 2.0 and is_bear_cross:
                                ...
                                clean_name = item.get("ticker", "").split('.')[0]
                                if shares_to_buy > 0: 
                                    buy_list.append(f"<li style='margin-bottom:12px; font-size:1.15rem; border-bottom:1px solid #e2e8f0; padding-bottom:8px;'>🛒 <span style='font-weight:900; width:120px; display:inline-block;'>{clean_name}</span>：下達 <span class='badge-buy'>🟢 BUY TO OPEN (加碼)</span> <span style='font-weight:900; margin-left:8px;'>{fmt_money(shares_to_buy)} 股</span> <span style='color:#64748b; font-size:0.95rem; margin-left:12px;'>(需資金 NTD {fmt_money(shares_to_buy * price_ntd)})</span></li>")
