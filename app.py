...
.stNumberInput input { font-weight: 800 !important; color: #f8fafc !important; font-size: 1.1rem !important; background-color: #0f172a;}
.modebar { display: none !important; }
hr { border-color: #334155; margin: 2rem 0; border-style: dashed; }
    ...
    try:
        ...
        if not df.empty:
            ...
            if not df.empty:
                ...
                # 🛡️ 吊燈停損線軌跡 (Chandelier Exit) - 供畫圖用 (22期最高 - 2.5ATR)
...
<div style='padding:14px; background:rgba(30,41,59,0.7); border:1px solid #334155; border-left:4px solid {breadth_color}; border-radius:8px; margin-bottom:14px;'>
    ...
    <div style='color:#f8fafc; font-size:1.05rem; font-weight:900;'>{breadth_status}</div>
</div>
""", unsafe_allow_html=True)
...
if today.day >= 10 and today.day <= 15:
    st.sidebar.warning("📅 **本週總經預警**：\n即將公布 CPI 數據，請留意市場雙向洗盤，嚴禁重壓高槓桿部位。")
...
if app_mode == "🏠 宏觀資產矩陣 (Dashboard)":
    ...
    # 頂部全域宏觀信號燈
    st.markdown(f"""
    <div style='display:flex; justify-content:space-between; background:#1e293b; padding:12px 24px; border-radius:8px; border:1px solid #334155; margin-bottom:20px;'>
        <span style='color:#f8fafc; font-weight:800;'>🟢 全域安全燈號：</span>
        <span>美債利差: <span style='color:{macro_color}; font-weight:900;'>{'正常' if yield_spread>0 else '倒掛警戒'}</span></span> | 
        <span>VIX 動能: <span style='color:{vix_color}; font-weight:900;'>{vix_status}</span></span> | 
        <span>SP500 寬度: <span style='color:{breadth_color}; font-weight:900;'>{'多頭' if market_breadth_bullish else '空頭'}</span></span>
    </div>
    """, unsafe_allow_html=True)
...
# ==========================================
    ...
    tab_monitor, tab_edit, tab_inject = st.tabs(["📊 動態監控矩陣 (Live)", "📓 量化覆盤與日誌 (Journal)", "💰 動態注水配置 (Inject)"])
        ...
        with st.spinner(f"🔄 正在同步雲端即時報價與風險 Beta 矩陣..."):
            for asset in target_portfolio:
                ...
                if m_data and m_data.get("price", 0) > 0:
                    ...
                    ann_roi = (((1 + net_pnl_pct / 100) ** (365 / h_days) - 1) * 100) if h_days > 0 else net_pnl_pct
                    ...
                    current_view_data.append({
                        **asset, "now_p": now_p, "date": date_str, 
                        ...
                        "drawdown": m_data.get("drawdown", 0), "max_since_buy": max_since_buy,
                        ...
                        "history_close": hist_close, "full_df": m_data.get("full_df")
                    })
        ...
        if st.button("✨ 讓 Gemini 檢討我的交易決策與備註", key=f"review_ai_btn_{market_label}", use_container_width=True):
            if not MY_API_KEY: st.warning("⚠️ 請先掛載 Gemini API Key。")
                ...
                with st.spinner("🧠 正在分析您的歷史備註與進出點位..."):
                    ...
                    prompt = f"你是嚴格的華爾街量化交易教練。這是我近期的歷史交易明細（包含我當時買賣的備註理由）：\n{history_str}\n我的總勝率是 {win_rate:.1f}%。請綜合評估我的「進出場理由（備註）」是否具有邏輯一致性，並給予我未來操作的毒舌指點與改進建議。請用專業繁體中文。"
                    try:
        ...
        if current_view_data:
            ...
            <div style='display:flex; gap: 16px; margin-bottom: 24px; flex-wrap:wrap;'>
                ...
                <div class='kpi-card' style='flex:1; min-width:180px; border-top: 4px solid #f59e0b; background-color:rgba(245,158,11,0.1);'>
                    <div class='data-label' style='color:#f59e0b;'>資產組合加權風險 (Beta)</div>
                ...
                <div class='kpi-card' style='flex:1; min-width:180px; border-top: 4px solid {pnl_color};'>
                    <div class='data-label'>未實現淨損益 (Unrealized PnL)</div>
            ...
            # 🦖 集中度預警
            if tech_ratio > 70:
                st.warning(f"⚠️ **濃度風險預警**：您的組合中，半導體與科技巨頭（如 2330, QQQ 等）的權重高達 **{tech_ratio:.1f}%**。一旦產業回調將面臨集體爆炸風險，建議適度分散或提高現金水位！")
            ...
            for item in current_view_data:
                ...
                if item.get("ticker") == "CASH":
                    ...
                    c[3].markdown(f"<div class='data-label'>風險波動係數 (Beta):</div><div class='data-value' style='color:#10b981;'>0.00x (避險)</div><div class='data-label' style='margin-top:12px;'>歷史已實現淨利 (Realized PnL):</div><div class='data-value' style='color:#10b981;'>NTD {fmt_money(item.get('realized_pnl', 0))}</div>", unsafe_allow_html=True)
                    ...
                    # ⏱️ 核心升級：時間價值與持倉殖利率
                    c[0].markdown(f"<div class='ticker-display'>{clean_name}</div><div class='stock-name-display'>{zh_name}</div><div class='price-display'>{'NTD' if is_tw_mode else 'USD'} {n_p:.2f}</div><div style='margin-top:6px; font-size:0.95rem; font-weight:900; color:#f8fafc;'>📦 庫存: {fmt_money(shares_qty)} 股</div><div class='date-display'>均價: {item.get('buy_price',0):.2f} (持倉 {item.get('holding_days', 0)} 天)</div>", unsafe_allow_html=True)
                    ...
                    c[2].markdown(f"<div class='data-label'>未實現淨損益 (Net Pnl):</div><div class='data-value' style='color:{item_pnl_color};'>{item_pnl_sign}{fmt_money(pnl_ntd)}</div><div class='data-label' style='margin-top:12px;'>年化回報率 (Ann. ROI):</div><div class='data-value' style='color:{item_pnl_color};'>{item_pnl_sign}{item.get('ann_roi', 0):.1f}%</div>", unsafe_allow_html=True)
                    ...
                    trail_dd_actual = ((n_p - max_since_buy) / max_since_buy * 100) if max_since_buy > 0 else 0.0
                    trail_color = "#ef4444" if is_trailing_stop else ("#f59e0b" if trail_dd_actual < -(atr_multiplier * atr_val / max(max_since_buy, 1) * 50) else "#94a3b8")
                    c[3].markdown(f"<div class='data-label'>技術趨勢 (50/200MA):</div><div>{trend_tag}</div><div class='data-label' style='margin-top:12px;'>距52週高點 (52W):</div><div class='data-value' style='color:#3b82f6;'>{item.get('dist_52w',0):.1f}%</div>", unsafe_allow_html=True)
                    ...
                    if is_trailing_stop:
                        tactical_action = f"<span style='color:#ef4444; font-weight:900;'>🚨 破 ATR 停利 (${stop_loss_price:.1f})</span>"
                    ...
                    c[4].markdown(f"<div class='data-label'>乖離率掃描 (BIAS):</div><div class='data-value'>{bias_val:+.1f}%</div><div class='data-label' style='margin-top:12px;'>🧠 終端戰術 (Tactics):</div><div style='font-size:0.95rem;'>{tactical_action}</div>", unsafe_allow_html=True)
                ...
                with c[5]:
                    ...
                    # 💡 加入持倉成本殖利率
                    progress_html = f"<div style='margin-top:8px; margin-bottom:6px; font-size:0.8rem; color:#94a3b8; font-weight:700;'>現時占比 {real_pct:.1f}% / 成本殖利率: <span style='color:#34d399;'>{yoc_val:.1f}%</span> / 目標 {new_tgt}%{vix_warn_str}</div><div style='width: 100%; background-color: #334155; border-radius: 99px; height: 10px; overflow:hidden;'><div style='width: {min(100, real_pct)}%; background-color: {'#10b981' if abs(diff) <= rebalance_threshold else '#f59e0b'}; height: 100%; border-radius: 99px;'></div></div>"
                ...
                if item.get("ticker") != "CASH":
                    with st.expander(f"📈 展開 {clean_name} 歷史淨利累積與 ATR 買賣點位標註 (Trade Annotations)"):
                        ...
                        if df_full is not None and not df_full.empty and 'MA1' in df_full.columns:
                            ...
                            if 'Chandelier_Exit' in df_full.columns:
                                fig_k.add_trace(go.Scatter(x=df_full.index, y=df_full['Chandelier_Exit'], mode='lines', name="ATR 停利防守線", line=dict(color='#f43f5e', width=1.5, dash='dot')), row=1, col=1)
    ...
    with tab_inject:
        ...
        # 💡 新增：階梯式資金注水模式選擇
        inject_mode = st.radio("注入資金演算法策略 (Pyramiding Mode)：", ["⚖️ 標準平衡模式 (依據目標權重)", "📈 右側順勢加碼 (僅加碼多頭強勢股，拒絕弱勢)", "📉 左側抄底建倉 (僅買進 RSI<40 之超跌股)"], horizontal=True)
        ...
        if add_cash > 0 and current_view_data:
            ...
            if not eligible_items:
                st.warning("⚠️ 在您選擇的策略模式下，目前沒有符合條件的標的可供注水！建議資金留存現金水位。")
            else:
                    ...
                    if shortfall_ntd > 0:
                        if item.get("ticker") == "CASH":
                            ...
                            if lev >= 2.0 and is_bear_cross:
                                ...
                                buy_list.append(f"<li style='margin-bottom:12px; font-size:1.15rem; border-bottom:1px solid #334155; padding-bottom:8px;'>🛒 <span style='font-weight:900; width:120px; display:inline-block;'>{item.get('ticker').split('.')[0]}</span>：本應分配 NTD {fmt_money(shortfall_ntd)}，但 <span style='color:#f59e0b; font-weight:800; background:rgba(245,158,11,0.2); padding:4px 10px; border-radius:6px;'>⚠️ 市場寬度背離 (破線)</span> 建議暫停放大槓桿，轉為現金。</li>")
                            else:
...
# ==========================================
# 💸 新增分頁 4: 現金流與稅務水庫 (Passive Cashflow & Tax Optimizer)
# ==========================================
elif app_mode == "💸 現金流與稅務水庫":
    st.markdown("<div class='market-header global-market' style='background: linear-gradient(135deg, #065f46 0%, #0f172a 100%); border-left-color: #10b981;'>💸 被動現金流水庫與稅務優化 (Cashflow Optimizer)</div>", unsafe_allow_html=True)
    
    st.info("💡 **模組說明**：系統將掃描您持倉中的配息紀錄與現時殖利率，為您精算長期複利效益，並防堵高額的二代健保補充保費。")
    ...
    for scheme_name in ["🎯 台股主力配置", "🎯 美股主力配置"]:
        ...
        for asset in agg_assets:
            ...
            if yield_val > 0 and price > 0:
                ...
                # 台灣專屬二代健保預警 (單次超過 20,000 元)
                # 假設台股高股息多為季配或月配，用全年/4 或 全年/12 粗估單次金額
                if is_tw:
                    freq = 4 if '00' in asset['ticker'] else 1 # 簡單假設 ETF 季配，個股年配
                    est_single_div = expected_annual_div / freq
                    if est_single_div > 20000:
                        total_tax_warning.append(f"⚠️ **{asset['ticker'].split('.')[0]}**：預估單次配息達 **NTD {fmt_money(est_single_div)}**，將被強制扣除 **2.11% 二代健保補充保費 (約 NTD {fmt_money(est_single_div * 0.0211)})**。建議留意除息前是否逢高獲利了結，或拆分至家屬帳戶。")

                all_div_records.append({
                    "資產代碼": asset['ticker'].split('.')[0],
                    "庫存市值 (NTD)": val_ntd,
                    "當前殖利率 (%)": yield_val * 100,
                    "成本殖利率 YoC (%)": asset.get('yoc', 0),
                    "預估未來12個月配息 (NTD)": expected_annual_div
                })
    ...
    col_kpi3.metric("被動收入可覆蓋生活費比率", f"{(monthly_cashflow / 50000)*100:.1f}%", "以每月基本開銷 5 萬計")
    
    st.markdown("---")
    
    st.subheader("📆 現金流貢獻矩陣")
    if all_div_records:
        df_div = pd.DataFrame(all_div_records)
        st.dataframe(df_div.style.format({"庫存市值 (NTD)": "{:,.0f}", "當前殖利率 (%)": "{:.2f}%", "成本殖利率 YoC (%)": "{:.2f}%", "預估未來12個月配息 (NTD)": "{:,.0f}"}), use_container_width=True)
    else:
        st.info("目前持倉中無穩定配息之資產。")
        
    st.markdown("<br>", unsafe_allow_html=True)
    
    if total_tax_warning:
        st.markdown(f"<div class='action-box' style='background:rgba(245,158,11,0.1); border:1px solid #b45309; border-left:6px solid #f59e0b;'><h4 style='color:#f59e0b;'>🚨 稅務漏洞預警 (Tax Leakage Warning)</h4><div style='color:#f8fafc; font-size:1rem; line-height:1.6;'>{'<br>'.join(total_tax_warning)}</div></div>", unsafe_allow_html=True)
        
    st.subheader("❄️ DRIP 股息再投資雪球兵推 (DRIP Simulation)")
    st.write("若您選擇將未來一年的預估股息全數投入大盤 (假設年化成長 8%)，10 年後這筆『免本金』的利息將滾出多少錢？")
    future_value = total_expected_div * (1.08 ** 10)
    st.success(f"📈 10 年後，光是今年度的配息，將為您額外滾出高達 **NTD {fmt_money(future_value)}** 的財富積累！請堅持長線投資的複利紀律。")
