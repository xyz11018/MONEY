...
# ==========================================
# 3. 📈 即時數據與量化指標抓取引擎 (提取歷史 K 線陣列供畫圖使用)
# ==========================================
...
if app_mode in ["🇹🇼 台股量化部位管理", "🇺🇸 美股量化部位管理"]:
    ...
    with st.expander(f"⚙️ 點此編輯交易日誌與庫存戰略", expanded=(not db_data["schemes"][current_scheme_name]["assets"])):
        st.info(f"💡 提示：輸入【買進均價】與【日期】可讓系統精算累積報酬率與走勢圖。現金不需填買進均價。")
    ...
    if target_portfolio:
        ...
        if not portfolio_history_df.empty:
            portfolio_history_df.fillna(method='ffill', inplace=True)
        ...
        if current_view_data:
            ...
            if not portfolio_history_df.empty:
                ...
                # 若開啟隱私模式，將絕對金額轉為「成長百分比 (Growth %)」圖表
                if privacy_mode:
        ...
        if st.button(f"✨ 啟動 Gemini 首席量化分析", key="manual_portfolio_ai_btn", type="primary", use_container_width=True):
            if not api_key: st.warning("⚠️ 請先在後台 Secrets 中設定 API Key。")
                ...
                with st.spinner("🧠 兵推引擎運算中 (Processing...)"):
                    # 💡 注意：傳給 AI 的 prompt 使用真實金額，以保證運算正確，但要求 AI 在輸出時避免提及絕對金額，保護隱私。
                    ...
                    privacy_instruction = "使用者目前開啟了【隱私防窺模式】，因此你的報告中**絕對不可以出現任何真實的金額數字 (如總市值、股數、台幣等)**，請一律使用「百分比 (%)」或是「部位比重」來做說明。" if privacy_mode else ""
