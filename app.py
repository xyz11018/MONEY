        ...
        if current_view_data:
            ...
            <div style='display:flex; gap: 16px; margin-bottom: 24px; flex-wrap:wrap;'>
                ...
                <div class='kpi-card' style='flex:1; min-width:180px; border-top: 4px solid #f59e0b; background-color:#fffbeb;'>
                    <div class='data-label' style='color:#b45309;'>組合加權波動風險 (Portfolio Beta)</div>
            ...
            if rebalance_orders:
                st.markdown(f"<div class='action-box' style='background:#ffffff; border:1px solid #e2e8f0; border-left:6px solid #f59e0b; padding:24px; border-radius:12px; margin-bottom:30px; box-shadow:0 4px 6px rgba(0,0,0,0.05);'><h4 style='color:#b45309; font-weight:900; margin-top:0; font-size:1.3rem; letter-spacing:0.5px;'>⚡ 演算法再平衡指令單 (Algorithmic Balancing Orders)</h4><div style='color:#64748b; margin-bottom:16px; font-size:0.95rem; font-weight:600;'>已綜合評估「資產偏離度」、「VIX 恐慌降載參數」與「均線過濾網」，請依據下表指引操作：</div><ul style='margin-bottom:0; padding-left:0; list-style-type:none;'>{''.join(rebalance_orders)}</ul></div>", unsafe_allow_html=True)
            else:
            ...
            for item in current_view_data:
                ...
                if item.get("ticker") == "CASH":
                    ...
                    is_bear = n_p < ma200_v
                    is_bear_cross = ma50_v < ma200_v
                    if not is_bear_cross: trend_tag = "<span style='color:#10b981; font-weight:900;'>🟢 多頭排列 (黃金交叉)</span>"
                    else: trend_tag = "<span style='color:#ef4444; font-weight:900;'>🔴 空頭防守 (死亡交叉)</span>"
                        
                    dd_color = "#ef4444" if item.get('drawdown', 0) < -20 else ("#f59e0b" if item.get('drawdown', 0) < -10 else "#64748b")
                    c[3].markdown(f"<div class='data-label'>技術面趨勢 (50/200MA):</div><div>{trend_tag}</div><div class='data-label' style='margin-top:12px;'>高點回撤 (Drawdown):</div><div class='data-value' style='color:{dd_color};'>{item.get('drawdown',0):.1f}%</div>", unsafe_allow_html=True)
                    
                    # 💡 核心優化：非對稱戰術濾網 (Buy the Dip)
                    ...
                    if is_bear_cross and lev >= 2.0: 
                        tactical_action = "<span style='color:#ef4444; font-weight:900;'>🛑 均線破線 (STOP LOSS)</span>"
                    elif not is_bear and (k_val < 25 or rsi_val < 35): 
                        tactical_action = "<span style='color:#10b981; font-weight:900;'>🟢 逢低抄底 (BUY THE DIP)</span>"
                    elif k_val > 80 or rsi_val > 75: 
                        tactical_action = "<span style='color:#f59e0b; font-weight:900;'>⚠️ 高檔過熱 (WATCH OUT)</span>"
                    elif bias_val >= 20: 
                        tactical_action = "<span style='color:#ef4444; font-weight:900;'>🚨 獲利解平 (TAKE PROFIT)</span>"
            ...
            if st.button(f"✨ 啟動 Gemini 演算法兵推", key="manual_portfolio_ai_btn", type="primary", use_container_width=True):
                if not api_key: st.warning("⚠️ 系統連線失敗：請先配置您的 Gemini API Key。")
                    ...
                    with st.spinner("🧠 機構級神經網絡交易模型推演中..."):
                        ...
                        prompt = f"你是對沖基金的首席量化操盤手。請根據下方市場動態與資產數據下達專業戰術指令：\n{portfolio_summary}\n請給出：1. 宏觀組合與總經風控診斷 2. 均線死叉/金叉標的精確操盤指令 3. 演算法再平衡平衡調配。用專業繁體金融術語回覆。"
                        try:
