...
# 💡 核心優化：支援「買/賣/配息」三向處理，並匯出戰績指標
def aggregate_lots(lots, targets):
    agg = {}
    perf = {"realized_pnl": 0.0, "total_div": 0.0, "wins": 0, "losses": 0}
    has_cash = False
    ...
    for lot in sorted_lots:
        ...
        if tk not in agg:
            agg[tk] = {"ticker": tk, "init_shares": 0.0, "total_cost": 0.0, "target_pct": targets.get(clean_tk, 0.0), "earliest_buy_date": None, "dividends": 0.0, "realized_pnl": 0.0}
        ...
        if action == "BUY":
            ...
            if tk == "CASH":
                ...
                if agg[tk]["init_shares"] > 0:
                    ...
                    if trade_pnl > 0: perf["wins"] += 1
                    elif trade_pnl < 0: perf["losses"] += 1
            ...
            # 領息不影響股數，price_val 直接當作領到的總現金
            agg[tk]["dividends"] += price_val
            perf["total_div"] += price_val

    if not has_cash: agg["CASH"] = {"ticker": "CASH", "init_shares": 0.0, "total_cost": 0.0, "target_pct": targets.get("CASH", 0.0), "earliest_buy_date": None, "dividends": 0.0, "realized_pnl": 0.0}
...
# ==========================================
                ...
                if m_data and m_data.get("price", 0) > 0:
                    ...
                    current_view_data.append({
                        **asset, "now_p": now_p, "date": date_str, 
                        ...
                        "earliest_buy_date": asset.get("earliest_buy_date"), "history_close": hist_close
                    })
    ...
    with tab_edit:
            ...
            if submit_quick_add:
                if qa_tk and (qa_shares > 0 or "DIVIDEND" in qa_action):
                    ...
                    if real_tk:
                        ...
                        save_portfolio(db_data)
                        st.success(f"✅ 交易確認已入帳：{resolved_name} ({real_tk}) {act_str}！")
                        st.rerun()
        ...
        # 🏆 AI 戰績檢討功能
        st.markdown("<hr>", unsafe_allow_html=True)
        st.subheader("🏆 AI 歷史戰績檢討與覆盤教練 (Performance Review)")
    ...
    with tab_monitor:
        if current_view_data:
            ...
            # 💡 核心新增：移動停利容忍度
            trail_threshold = c_slider2.slider("📉 吊燈移動停利容忍度 (Trailing Stop %)", 5.0, 40.0, 15.0, 1.0)
            ...
            for item in current_view_data:
                ...
                # 🛡️ 移動停利檢查優先級最高！
                is_trailing_stop = (trail_dd < -trail_threshold) and (item.get("ticker") != "CASH") and (item.get("init_shares") > 0)
                
                if is_trailing_stop:
                    rebalance_orders.append(f"<li style='margin-bottom:12px; font-size:1.1rem; border-bottom:1px solid #e2e8f0; padding-bottom:8px;'>📉 <b>{clean_name}</b> ➡️ 已從建倉後高點回撤 <b>{trail_dd:.1f}%</b>，觸發 <span style='color:#991b1b; font-weight:900; background:#fef2f2; padding:4px 10px; border-radius:6px; margin-right:12px; border:1px solid #fecaca;'>🚨 移動停利/停損機制</span> 建議無條件 <span style='color:#991b1b; font-weight:900; background:#fee2e2; padding:4px 10px; border-radius:6px;'>全數平倉 (SELL ALL)</span> 保護資本！</li>")
                    ...
                    if item.get("ticker") == "CASH":
                        ...
                        if shares_diff > 0:
                            if lev >= 2.0 and is_bear_cross:
                                rebalance_orders.append(f"<li style='margin-bottom:12px; font-size:1.1rem; border-bottom:1px solid #e2e8f0; padding-bottom:8px;'>🛒 <b>{clean_name}</b> ➡️ 本應加碼，但目前 <span style='color:#b45309; font-weight:900; background:#fffbeb; padding:4px 10px; border-radius:6px; margin-right:12px;'>⚠️ 雙均線死叉（空頭防守）</span> <b>建議暫緩建倉</b>。</li>")
                            else:
                            ...
                            if lev >= 2.0 and is_bear_cross:
                                rebalance_orders.append(f"<li style='margin-bottom:12px; font-size:1.1rem; border-bottom:1px solid #e2e8f0; padding-bottom:8px;'>📉 <b>{clean_name}</b> ➡️ 比例過高且 <span style='color:#991b1b; font-weight:900; background:#fef2f2; padding:4px 10px; border-radius:6px; margin-right:12px; border:1px solid #fecaca;'>🚨 雙均線死叉</span> 建議加速 <span style='color:#991b1b; font-weight:900; background:#fee2e2; padding:4px 10px; border-radius:6px; margin-right:12px;'>執行平倉 (SELL)</span> <b>{fmt_money(abs(shares_diff))} 股</b></li>")
                            else:
                                rebalance_orders.append(f"<li style='margin-bottom:12px; font-size:1.1rem; border-bottom:1px solid #e2e8f0; padding-bottom:8px;'>📉 <b>{clean_name}</b> ➡️ 比例過高{vix_warning_text}，建議 <span style='color:#991b1b; font-weight:900; background:#fee2e2; padding:4px 10px; border-radius:6px; margin-right:12px;'>執行平倉 (SELL)</span> <b>{fmt_money(abs(shares_diff))} 股</b></li>")
            ...
            for item in current_view_data:
                ...
                if item.get("ticker") == "CASH":
                    ...
                    # 💡 核心變更：顯示建倉以來的 Trailing Drawdown
                    trail_color = "#ef4444" if trail_dd < -trail_threshold else ("#f59e0b" if trail_dd < -(trail_threshold/2) else "#64748b")
                    c[3].markdown(f"<div class='data-label'>技術趨勢 (50/200MA):</div><div>{trend_tag}</div><div class='data-label' style='margin-top:12px;'>建倉後最高回撤 (Trail DD):</div><div class='data-value' style='color:{trail_color};'>{trail_dd:.1f}%</div>", unsafe_allow_html=True)
                    ...
                    if trail_dd < -trail_threshold:
                        tactical_action = "<span style='color:#ef4444; font-weight:900;'>🚨 觸發移動停利 (SELL ALL)</span>"
                ...
                with c[5]:
                    ...
                    if trail_dd < -trail_threshold and item.get("ticker") != "CASH":
            ...
            st.info("💡 點擊下方按鈕，AI 將綜合審視您的「美債利差狀態」、「均線交叉」與「倉位偏離度」，下達頂級決策邏輯。")
