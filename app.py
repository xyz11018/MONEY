...
st.markdown("""
    ...
    .kpi-card {
        background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px;
        padding: 20px; box-shadow: 0 2px 5px rgba(0,0,0,0.02); display: flex; flex-direction: column; justify-content: center;
    }
...
if current_vix >= 30: vix_color, vix_status = "#10b981", "🟢 恐慌 (尋找買點)"
elif current_vix <= 12: vix_color, vix_status = "#ef4444", "🔴 極低 (居高思危)"
...
cnn_val = 65 
if cnn_val >= 80: cnn_color, cnn_status = "#ef4444", "🔴 極度貪婪 (切勿追高)"
elif cnn_val >= 60: cnn_color, cnn_status = "#f59e0b", "🟡 貪婪"
elif cnn_val <= 30: cnn_color, cnn_status = "#10b981", "🟢 恐懼 (尋找買點)"
else: cnn_color, cnn_status = "#64748b", "🔵 中性"
st.sidebar.markdown(f"🦅 **CNN 恐懼貪婪：** <span style='color:{cnn_color}; font-weight:bold;'>{cnn_val} ({cnn_status})</span>", unsafe_allow_html=True)

tw_light_signal = "🟢 綠燈 (31分)"  
tw_light_color = "#10b981"  
st.sidebar.markdown(f"🚦 **景氣對策信號：** <span style='color:{tw_light_color}; font-weight:bold;'>{tw_light_signal}</span>", unsafe_allow_html=True)
st.sidebar.markdown(f"<div style='font-size:0.75rem; color:#94a3b8; margin-top:-10px; margin-bottom:10px;'>💡 戰略: 藍燈建倉、紅燈調節</div>", unsafe_allow_html=True)
...
if app_mode in ["🇹🇼 台股量化部位管理", "🇺🇸 美股量化部位管理"]:
    ...
    with st.expander(f"⚙️ 點此編輯庫存與戰略權重", expanded=(not db_data["schemes"][current_scheme_name]["assets"])):
        st.info(f"💡 輸入持股與【戰略目標權重%】（如 40, 40, 20），系統將自動為您監控偏離度。")
        cols = st.columns([2, 2, 2])
        cols[0].markdown("**代碼 或 名稱 (Ticker)**")
        cols[1].markdown("**持有數量 (Shares/Cash)**")
        cols[2].markdown("**戰略目標權重 (Target %)**")
        
        new_setup = []
        for i in range(int(num_assets)):
            r_cols = st.columns([2, 2, 2])
            hist = db_data["schemes"][current_scheme_name]["assets"][i] if i < len(db_data["schemes"][current_scheme_name]["assets"]) else {"ticker": "", "init_shares": 0, "target_pct": 0}
    ...
    if target_portfolio:
        ...
        if current_view_data:
            ...
            <div style='display:flex; gap:16px; margin-bottom:24px; flex-wrap:wrap;'>
                <div class='kpi-card' style='flex:1; min-width:250px; border-left: 5px solid #3b82f6;'>
                    <div class='data-label'>總投資市值 (Total AUM)</div>
                    <div style='font-size:2rem; font-weight:900; color:#0f172a;'>NTD {int(local_total_val):,}</div>
                </div>
                <div class='kpi-card' style='flex:1; min-width:250px; border-left: 5px solid #10b981;'>
                    <div class='data-label'>預估年度被動收入 (Est. Dividend)</div>
                    <div style='font-size:2rem; font-weight:900; color:#0f172a;'>NTD {int(local_total_dividend):,}</div>
                </div>
                <div class='kpi-card' style='flex:1; min-width:250px; border-left: 5px solid #8b5cf6;'>
                    <div class='data-label'>整體平均殖利率 (Avg. Yield)</div>
                    <div style='font-size:2rem; font-weight:900; color:#0f172a;'>{avg_div_rate:.2f}%</div>
                </div>
            </div>
            """
            ...
            for item in current_view_data:
                ...
                if item["ticker"] == "CASH":
                    ...
                    c[1].markdown(f"<div class='data-label'>持有金額:</div><div class='data-value'>{int(item.get('init_shares', 0)):,} 元</div><div class='data-label' style='margin-top:10px;'>真實市值:</div><div class='data-value'>NTD {int(item['now_val_ntd']):,}</div>", unsafe_allow_html=True)
                    ...
                    if diff_amt > 0: action_msg = f"<div class='badge-buy'>ADD 存入</div> <span style='font-weight:800; font-size:1.2rem; color:#0f172a; margin-left:8px;'>{diff_amt:,} {unit}</span>"
                    elif diff_amt < 0: action_msg = f"<div class='badge-sell'>SUB 提領</div> <span style='font-weight:800; font-size:1.2rem; color:#0f172a; margin-left:8px;'>{abs(diff_amt):,} {unit}</span>"
                    ...
                    c[1].markdown(f"<div class='data-label'>{'📊 投入金額:' if item['ticker'].startswith('^') else '持有數量:'}</div><div class='data-value'>{int(item.get('init_shares', 0)):,} {'元' if item['ticker'].startswith('^') else '股'}</div><div class='data-label' style='margin-top:10px;'>真實市值:</div><div class='data-value'>NTD {int(item['now_val_ntd']):,}</div>", unsafe_allow_html=True)
                    c[2].markdown(f"<div class='data-label'>資產佔比監控:</div>{progress_html}", unsafe_allow_html=True)
                    ...
                    if shares_diff > 0: action_msg = f"<div class='badge-buy'>BUY 買進</div> <span style='font-weight:900; font-size:1.2rem; color:#0f172a; margin-left:8px;'>{shares_diff:,} 股</span>"
                    elif shares_diff < 0: action_msg = f"<div class='badge-sell'>SELL 賣出</div> <span style='font-weight:900; font-size:1.2rem; color:#0f172a; margin-left:8px;'>{abs(shares_diff):,} 股</span>"
            ...
            for item in current_view_data:
                ...
                if shortfall_ntd > 0:
                    if item["ticker"] == "CASH":
                        ...
                        buy_list.append(f"💵 **現金保留**：**{int(buy_units):,}** {'元' if is_tw_mode else '美元'}")
                        ...
                        clean_name = item["ticker"].split('.')[0]
                        if shares_to_buy > 0: buy_list.append(f"🛒 **{clean_name}**：買進 **{shares_to_buy:,}** 股 (約 NTD {int(shares_to_buy * price_ntd):,})")
            if buy_list:
        ...
        if st.button(f"✨ 啟動 Gemini 首席量化分析，分析【{current_scheme_name}】", key="manual_portfolio_ai_btn", type="primary", use_container_width=True):
                ...
                with st.spinner("載入量化數據與繪製圖表中..."):
                    ...
                    if not df.empty:
                        ...
                        if ticker_input.startswith("^"):
                            cc1.markdown(f"<div class='pro-card'><div class='data-label'>📈 指數最新收盤價位</div><div class='data-value' style='font-size:1.6rem;'>NTD/USD {last_close:,.2f}</div><div style='color:#64748b; font-size:0.85rem; margin-top:8px;'>長線均線({n3}): {ma200_val:,.2f}</div></div>", unsafe_allow_html=True)
                            cc2.markdown(f"<div class='pro-card'><div class='data-label'>📉 歷史高點與波段回撤</div><div class='data-value' style='font-size:1.6rem;'>回撤率: {dd_pct:.2f}%</div><div style='color:#64748b; font-size:0.85rem; margin-top:8px;'>最高位階: {high_52w:,.2f}</div></div>", unsafe_allow_html=True)
