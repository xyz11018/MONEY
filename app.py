...
# ==========================================
# 1. 頂級法人看盤室現代化 SaaS CSS 視覺系統 (修復對比度)
# ==========================================
...
def fetch_market_data(ticker):
    ...
    try:
        ...
        if not df.empty:
            ...
            if not df.empty:
                # 🛡️ 修復 KeyError: 強制寫入 MA 指標進 DataFrame，供後續繪圖使用
        ...
        if realtime_price > 0:
            ...
            fallback_df['MA1'], fallback_df['MA2'], fallback_df['MA3'] = realtime_price, realtime_price, realtime_price
            return {
...
<div style='padding:14px; background:#f8fafc; border:1px solid #e2e8f0; border-left:4px solid {breadth_color}; border-radius:8px; margin-bottom:14px; box-shadow: 0 1px 2px rgba(0,0,0,0.02);'>
    <div style='color:#64748b; font-size:0.75rem; font-weight:800; margin-bottom:6px; text-transform:uppercase; letter-spacing:1px;'>🕸️ 市場寬度 (S&P500)</div>
    <div style='color:#0f172a; font-size:1.1rem; font-weight:900;'>{breadth_status}</div>
</div>
""", unsafe_allow_html=True)
        ...
        <div style='display:flex; flex-direction:column; gap:12px;'>
            ...
            <div style='display:flex; gap:12px;'>
                ...
                <div class='kpi-card' style='flex:1; padding:12px;'>
                    ...
                    <div style='font-size:1.3rem; font-weight:900; color:#0f172a;'>NTD {fmt_money(total_div_ntd)}</div>
                </div>
            </div>
        </div>
        """
...
# ==========================================
    ...
    with tab_edit:
        ...
        with st.form(key=f"quick_add_form_{market_label}"):
            ...
            qa_price = qa_cols[3].number_input("單價/總息", min_value=0.0, step=1.0, help="若為領息，請輸入總金額")
        ...
        c_perf4.metric("⚖️ 凱利公式建議資金上限", f"{tab_perf['kelly_pct']:.1f}%", "單檔合理最高押注比例")
    ...
    with tab_monitor:
        if current_view_data:
            ...
            for item in current_view_data:
                ...
                if is_trailing_stop:
                    rebalance_orders.append(f"<li style='margin-bottom:12px; font-size:1.1rem; border-bottom:1px solid #e2e8f0; padding-bottom:8px;'>📉 <b>{clean_name}</b> ➡️ 價格跌破 ATR 吊燈線 <b>{stop_loss_price:.2f}</b>，觸發 <span style='color:#ef4444; font-weight:900; background:rgba(239,68,68,0.2); padding:4px 10px; border-radius:6px; margin-right:12px; border:1px solid #b91c1c;'>🚨 ATR 移動停利/停損</span> 建議無條件 <span style='color:#ef4444; font-weight:900; background:rgba(239,68,68,0.2); padding:4px 10px; border-radius:6px;'>全數平倉 (SELL ALL)</span> 保護資本！</li>")
                    ...
                    if item.get("ticker") == "CASH":
                        ...
                        if shares_diff > 0:
                            if lev >= 2.0 and is_bear_cross:
                                rebalance_orders.append(f"<li style='margin-bottom:12px; font-size:1.1rem; border-bottom:1px solid #e2e8f0; padding-bottom:8px;'>🛒 <b>{clean_name}</b> ➡️ 本應加碼，但目前 <span style='color:#f59e0b; font-weight:900; background:#fffbeb; padding:4px 10px; border-radius:6px; margin-right:12px;'>⚠️ 雙均線死叉（空頭防守）</span> <b>建議暫緩建倉</b>。</li>")
                            elif not market_breadth_bullish and lev >= 2.0:
                                rebalance_orders.append(f"<li style='margin-bottom:12px; font-size:1.1rem; border-bottom:1px solid #e2e8f0; padding-bottom:8px;'>🛒 <b>{clean_name}</b> ➡️ 本應加碼，但目前 <span style='color:#f59e0b; font-weight:900; background:#fffbeb; padding:4px 10px; border-radius:6px; margin-right:12px;'>⚠️ 市場寬度背離 (破線)</span> <b>建議暫停放大槓桿</b>。</li>")
                            else:
                                rebalance_orders.append(f"<li style='margin-bottom:12px; font-size:1.1rem; border-bottom:1px solid #e2e8f0; padding-bottom:8px;'>🛒 <b>{clean_name}</b> ➡️ 比例不足{vix_warning_text}，建議 <span style='color:#166534; font-weight:900; background:#dcfce7; padding:4px 10px; border-radius:6px; margin-right:12px; border:1px solid #a7f3d0;'>執行建倉 (BUY)</span> <b>{fmt_money(shares_diff)} 股</b> <span style='color:#64748b; font-size:0.9rem;'>(約 NTD {fmt_money(shares_diff * price_ntd)})</span></li>")
                        elif shares_diff < 0:
                            if lev >= 2.0 and is_bear_cross:
                                rebalance_orders.append(f"<li style='margin-bottom:12px; font-size:1.1rem; border-bottom:1px solid #e2e8f0; padding-bottom:8px;'>📉 <b>{clean_name}</b> ➡️ 比例過高且 <span style='color:#ef4444; font-weight:900; background:rgba(239,68,68,0.2); padding:4px 10px; border-radius:6px; margin-right:12px; border:1px solid #fecaca;'>🚨 雙均線死叉</span> 建議加速 <span style='color:#ef4444; font-weight:900; background:rgba(239,68,68,0.2); padding:4px 10px; border-radius:6px; margin-right:12px;'>執行平倉 (SELL)</span> <b>{fmt_money(abs(shares_diff))} 股</b></li>")
                            else:
                                rebalance_orders.append(f"<li style='margin-bottom:12px; font-size:1.1rem; border-bottom:1px solid #e2e8f0; padding-bottom:8px;'>📉 <b>{clean_name}</b> ➡️ 比例過高{vix_warning_text}，建議 <span style='color:#ef4444; font-weight:900; background:rgba(239,68,68,0.2); padding:4px 10px; border-radius:6px; margin-right:12px;'>執行平倉 (SELL)</span> <b>{fmt_money(abs(shares_diff))} 股</b></li>")
            
            if rebalance_orders:
                st.markdown(f"<div class='action-box' style='background:#ffffff; border:1px solid #e2e8f0; border-left:6px solid #f59e0b; padding:24px; border-radius:12px; margin-bottom:30px; box-shadow:0 4px 6px rgba(0,0,0,0.05);'><h4 style='color:#b45309; font-weight:900; margin-top:0; font-size:1.3rem; letter-spacing:0.5px;'>⚡ 演算法再平衡與移動停利指令單 (Algorithmic Orders)</h4><div style='color:#64748b; margin-bottom:16px; font-size:0.95rem; font-weight:600;'>已綜合評估「移動停利」、「資產偏離度」、「VIX 降載」與「均線網」，請依據下表操作：</div><ul style='margin-bottom:0; padding-left:0; list-style-type:none;'>{''.join(rebalance_orders)}</ul></div>", unsafe_allow_html=True)
            else:
                st.markdown(f"<div class='action-box' style='background:#f0fdf4; border:1px solid #bbf7d0; border-left:6px solid #10b981; padding:24px; border-radius:12px; margin-bottom:30px; box-shadow:0 4px 6px rgba(0,0,0,0.05);'><h4 style='color:#166534; font-weight:900; margin-top:0; font-size:1.3rem; letter-spacing:0.5px;'>✅ 資金配置穩健且未觸及停利 (Risk Balanced)</h4><div style='color:#166534; font-size:1rem; font-weight:600;'>目前全體底層現貨與槓桿部位均完美收斂於演算法安全區間。</div></div>", unsafe_allow_html=True)
            ...
            for item in current_view_data:
                ...
                if item.get("ticker") == "CASH":
                    ...
                    c[0].markdown(f"<div class='ticker-display'>{clean_name}</div><div class='stock-name-display'>{zh_name}</div><div class='price-display'>{'NTD' if is_tw_mode else 'USD'} {n_p:.2f}</div><div style='margin-top:6px; font-size:0.95rem; font-weight:900; color:#0f172a;'>📦 庫存: {fmt_money(shares_qty)} 股</div><div class='date-display'>均價: {item.get('buy_price',0):.2f}</div>", unsafe_allow_html=True)
                    ...
                    trail_color = "#ef4444" if is_trailing_stop else ("#f59e0b" if trail_dd < -(atr_multiplier * atr_val / max(max_since_buy, 1) * 50) else "#64748b")
                ...
                with c[5]:
                    ...
                    if is_trailing_stop and item.get("ticker") != "CASH":
                        box_bg, box_border, title_color, title_text = "#fef2f2", "#fecaca", "#991b1b", "🚨 強制移動停利平倉"
                    
                    vix_warn_str = f" <span style='color:#ef4444;'>(VIX降載 ➡️ {dynamic_tgt_p}%)</span>" if dynamic_tgt_p != new_tgt else ""
                    progress_html = f"<div style='margin-top:8px; margin-bottom:6px; font-size:0.8rem; color:#64748b; font-weight:700;'>現時占比 {real_pct:.1f}% / 風險 Beta: {lev:.1f}x / 原目標 {new_tgt}%{vix_warn_str}</div><div style='width: 100%; background-color: #e2e8f0; border-radius: 99px; height: 10px; overflow:hidden;'><div style='width: {min(100, real_pct)}%; background-color: {'#10b981' if abs(diff) <= rebalance_threshold else '#f59e0b'}; height: 100%; border-radius: 99px;'></div></div>"
                    ...
                    if item.get("ticker") == "CASH":
                        ...
                        shares_diff = int(diff_val / price_ntd) if price_ntd > 0 else 0
                        if is_trailing_stop:
                            action_msg = f"<div style='margin-top:16px;'><span class='badge-sell' style='background:#991b1b; color:white;'>SELL ALL 清倉停利</span> <span style='font-weight:900; font-size:1.15rem; color:#0f172a; margin-left:8px;'>{fmt_money(shares_qty)} 股</span></div>"
                        elif shares_diff > 0: 
                            if lev >= 2.0 and is_bear_cross: action_msg = f"<div style='margin-top:16px;'><span class='badge-hold'>均線死叉暫緩</span> <span style='font-weight:900; font-size:1.15rem; color:#0f172a; margin-left:8px;'>{fmt_money(shares_diff)} 股</span></div>"
                            else: action_msg = f"<div style='margin-top:16px;'><span class='badge-buy'>BUY 建倉</span> <span style='font-weight:900; font-size:1.15rem; color:#0f172a; margin-left:8px;'>{fmt_money(shares_diff)} 股</span></div>"
            ...
            for item in current_view_data:
                ...
                if shortfall_ntd > 0:
                    if item.get("ticker") == "CASH":
                        ...
                        if lev >= 2.0 and is_bear_cross:
                            buy_list.append(f"<li style='margin-bottom:12px; font-size:1.15rem; border-bottom:1px solid #e2e8f0; padding-bottom:8px;'>🛒 <span style='font-weight:900; width:120px; display:inline-block;'>{item.get('ticker').split('.')[0]}</span>：本應分配 NTD {fmt_money(shortfall_ntd)}，但 <span style='color:#b45309; font-weight:800; background:#fffbeb; padding:4px 10px; border-radius:6px;'>⚠️ 雙均線死叉（空頭防守）</span> 建議將此筆資金轉入現金水位保留。</li>")
                        elif not market_breadth_bullish and lev >= 2.0:
                            buy_list.append(f"<li style='margin-bottom:12px; font-size:1.15rem; border-bottom:1px solid #e2e8f0; padding-bottom:8px;'>🛒 <span style='font-weight:900; width:120px; display:inline-block;'>{item.get('ticker').split('.')[0]}</span>：本應分配 NTD {fmt_money(shortfall_ntd)}，但 <span style='color:#f59e0b; font-weight:800; background:#fffbeb; padding:4px 10px; border-radius:6px;'>⚠️ 市場寬度背離 (SP500 破線)</span> 建議暫停放大槓桿，轉為現金。</li>")
                        else:
